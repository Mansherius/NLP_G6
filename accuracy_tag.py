...
print(f'Tag-wise Accuracy for tag: {accuracy * 100:.2f}%')

# Generate confusion matrix
conf_matrix = confusion_matrix(tag1_encoded, tag2_encoded)
print('\nConfusion Matrix for tag:')
print(conf_matrix)
...
conf_matrix_df.to_csv('/Users/manshersingh/Documents/Ashoka Coursework/NLP - 6th Sem/POS_comparison_nltk/confusion_matrix_Tag_nltk.csv')

# Find and save mismatched labels to a file
mismatched_labels = [(label_encoder.classes_[t1], label_encoder.classes_[t2]) for t1, t2 in zip(tag1_encoded, tag2_encoded) if t1 != t2]
mismatched_labels_df = pd.DataFrame(mismatched_labels, columns=['True_Label', 'Predicted_Label'])
mismatched_labels_df.to_csv('/Users/manshersingh/Documents/Ashoka Coursework/NLP - 6th Sem/POS_comparison_nltk/mismatched_labels_Tag_nltk.csv')
